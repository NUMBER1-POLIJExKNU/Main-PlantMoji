...
| 16 | `milestone16-evolution-ladder.sql` | 10-stage companion ladder (Seed → … → Legend) + display-only care/affinity/day progress counters |
...
| 21 | `milestone21-level-evolution.sql` | level-only companion evolution (Lv.1 Seed → Lv.10+ Legend), atomic Bond Level sync + existing-player backfill |

- [ ] On the team's existing Supabase project, **milestone1–milestone8 are typically already applied** (the running game depends on them). **milestone9–milestone21 are newer** — verify milestone9 through milestone21 before go-live.
...
- [ ] Milestone 21 replaces care/day evolution gates with the single Bond Level rule and backfills existing companion stages. Run it after milestone16.
...
- [ ] `DEMO_CHEAT_CODE` *(opsional)* — kode untuk Settings → Demo Control Center ("Buka semuanya" = pameran Lv.10 yang aman diulang; "Kembali ke awal" = kembali ke Lv.1 / 0 XP). Default `admin` jika kosong; isi hanya jika ingin kode lain. Diperiksa di sisi server, tidak pernah terkirim ke browser.
...
| 16 | `milestone16-evolution-ladder.sql` | tangga companion 10 tahap (Seed → … → Legend) + penghitung progres perawatan/afinitas/hari khusus tampilan |
...
| 21 | `milestone21-level-evolution.sql` | evolusi companion hanya berdasarkan level (Lv.1 Seed → Lv.10+ Legend), sinkronisasi Bond Level + backfill pemain lama |

- [ ] Pada proyek Supabase tim yang sudah berjalan, **milestone1–milestone8 biasanya sudah diterapkan** (game yang berjalan bergantung padanya). **milestone9–milestone21 yang lebih baru** — pastikan milestone9 sampai milestone21 sebelum go-live.
...
- [ ] Milestone 21 mengganti syarat evolusi perawatan/hari dengan satu aturan Bond Level dan menyesuaikan tahap pemain lama. Jalankan setelah milestone16.
...
**Adegan 4 — Jelajah & Harta (penutup opsional).** Pindah ke **Jelajah** (`/plants`), pindai potret sensor nyata yang sama, dan buka penjelasan yang berdasar untuk ketidakcocokan terukur terbesar dibanding referensi tanaman Jember. Pindah ke **Harta** (`/collection`) dan ketuk Mood atau Lencana. Untuk **Evolusi Companion**, tunjukkan roadmap: Lv.1 adalah Seed, setiap Bond Level hingga Lv.10 membuka tahap visual berikutnya, dan layar menampilkan level berikut serta XP yang tersisa.
...
- [ ] `DEMO_CHEAT_CODE` *(선택)* — Settings → Demo Control Center 코드 ("Unlock everything" = 재실행 안전한 Lv.10 쇼케이스; "Reset to start" = Lv.1 / 0 XP로 복원). 비워두면 기본값 `admin`; 다른 코드를 쓰고 싶을 때만 설정하세요. 서버에서만 검사하며 브라우저에 노출되지 않습니다.
...
| 16 | `milestone16-evolution-ladder.sql` | 10단계 컴패니언 성장 사다리(Seed → … → Legend) + 표시 전용 돌봄/환경 종류/활동 일수 카운터 |
...
| 21 | `milestone21-level-evolution.sql` | 레벨 전용 컴패니언 진화(Lv.1 Seed → Lv.10+ Legend), Bond Level 동기화 + 기존 사용자 단계 이관 |

- [ ] 팀이 이미 운영 중인 Supabase 프로젝트라면 **milestone1–milestone8은 보통 이미 적용되어 있습니다** (돌아가는 게임이 이에 의존). **milestone9–milestone21이 더 최신** — milestone9부터 milestone21까지 고라이브 전에 확인하세요.
...
- [ ] Milestone 21은 돌봄 횟수·활동일 진화 조건을 Bond Level 단일 규칙으로 교체하고 기존 컴패니언 단계도 이관합니다. milestone16 다음에 실행하세요.
