...
.DESCRIPTION
    ...
    (npm run dev), narrating each step and waiting where the real demo waits:
      ...
      Baseline   Jin - Happy, Bond Lv.2 - 70 XP
      Scene 1    Overheating event (34.2 C) -> web shows Jin on fire + NEW QUEST
                 "Cool Me Down" (+30 XP reward)
      ...
      Scene 3    POST /api/game-tick polled every 30 s (up to 6 minutes) so the
                 time-window completion lands promptly: Quest Complete +30 XP,
                 70 + 30 = 100 XP -> LEVEL UP -> Bond Lv.3
...
Write-Host ""
Write-Host "Scenario baseline on camera: 'Plant Jin - Happy / Bond Lv.2 - 70 XP'."
Write-Host "The quest always completes with +30 XP; the LEVEL UP overlay appears only"
Write-Host "when XP crosses 100 (70 + 30 = 100 -> Bond Lv.3), so verify the XP counter"
Write-Host "on the web page before filming."
...
Write-Host "  Setting the on-camera starting point: Jin is Happy."
if (-not (Send-DemoEvent -LabelSlug "baseline-happy" -Data @{ currentState = "Happy" })) { exit 1 }
Write-Note "Confirm on the web page: Jin - Happy, and the XP counter you want on film (scenario assumes Bond Lv.2 - 70 XP)."
...
Write-Note "watch the web page: Jin turns 🔥 + Cool Me Down quest appears"
Write-Note "NEW QUEST 'Cool Me Down' — Reward +30 XP. Film this beat before recovering."
...
if ($FastForward) {
    ...
    Write-Host "    - Operator cue at that moment: 'Quest Complete +30 XP' on the web page;" -ForegroundColor DarkGray
    Write-Host "      70 + 30 = 100 XP -> LEVEL UP overlay -> Bond Lv.3 (physical celebration: RGB + buzzer + LCD)." -ForegroundColor DarkGray
    Write-Host ""
...
while (-not $completed -and (Get-Date) -lt $deadline) {
    ...
    elseif ($elapsed -ge 300) {
        ...
        Write-Note "The quest should now COMPLETE: watch the web page for 'Quest Complete +30 XP'."
        Write-Note "If the baseline was 70 XP: 70 + 30 = 100 XP -> LEVEL UP overlay -> Bond Lv.3 (physical celebration: RGB + buzzer + LCD)."
        $completed = $true
    }
    else {
...
Write-Host "is the whole pitch on camera — reset Jin to Happy and the target XP before the real take." -ForegroundColor Green
exit 0
