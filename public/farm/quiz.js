(() => {
  ...
  let questions=[],progress=[],index=0,round=0,busy=false,returnFocus=null,timer=null,timeLeft=15;
  const copy=()=>C[locale()];
  ...
  function render(){
    ...
    $("#quiz-title").textContent=c.title;$("#quiz-step").textContent=`${Math.min(index+1,3)} / 3${round?` · R${round+1}`:""}`;$("#quiz-meter-fill").style.width=`${Math.min(3,done.size)/3*100}%`;
    $("#quiz-feedback").hidden=true;next.hidden=true;next.dataset.practice="";
    if(!q){stopTimer();$("#quiz-question").textContent=c.summary(totalXp());$("#quiz-choices").replaceChildren();const f=$("#quiz-feedback");f.hidden=false;f.className="quiz-feedback correct";f.textContent="🌟 "+(round===0?c.done:c.summary(totalXp()));next.hidden=false;next.textContent=c.practice;next.dataset.practice="true";return;}
  ...
  function updateXpHud(data){if(data.total_xp==null)return;const total=Number(data.total_xp);const coin=$(".badge.coin");if(coin)coin.innerHTML=`<i class="icon">⭐</i> ${total} XP`;const bar=$(".xp-bar");if(bar)bar.style.width=`${total%100}%`;}
  ...
  async function answer(answerIndex,button,timedOut){
    ...
    try{const q=questions[index];const r=await fetch("/api/daily-quiz",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({plantId:"plant-01",questionKey:q.key,answerIndex,locale:locale(),round})});const d=await r.json();if(!d.ok)throw 0;const f=$("#quiz-feedback");f.hidden=false;
      ...
      else{button?.classList.add("wrong");f.className="quiz-feedback retry";f.textContent=`${timedOut?copy().timeout:copy().again} ${d.explanation}`;window.PMSfx?.play("tick");xpPod(d.xp_awarded,button);setTimeout(()=>updateXpHud(d),780);setTimeout(()=>{document.querySelectorAll(".quiz-choice").forEach(b=>b.disabled=false);startTimer();},650);}
