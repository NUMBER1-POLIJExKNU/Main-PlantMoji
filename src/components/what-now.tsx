...
export default function WhatNow({ locale, mood, questStatus }: { locale: AppLocale; mood: PlantMood; questStatus?: string | null }) {
  ...
  return <details className="pm-what-now"><summary>❓ {locale === "id" ? "SEKARANG APA?" : "WHAT NOW?"}</summary><p>{advice}</p><Link href={href}>{locale === "id" ? "TUNJUKKAN" : "SHOW ME"} →</Link></details>;
}
