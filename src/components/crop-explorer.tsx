...
// the emoji stays as the fallback if a file ever goes missing.
const PARAMS = {
  temperature: ["🌡️", "Suhu", "Temperature", "/icons/temperature.png"],
  airHumidity: ["💧", "Kelembapan udara", "Air humidity", "/icons/humidity.png"],
  light: ["☀️", "Cahaya", "Light", "/icons/brightness.png"],
  soilPh: ["🧪", "pH tanah", "Soil pH", "/icons/acidity.png"],
} as const;

/** Sized in em so it matches whatever size the emoji it replaced rendered at. */
function ParamIcon({ src }: { src: string }) {
  // eslint-disable-next-line @next/next/no-img-element -- same-origin pixel art; the optimizer would resample the crisp pixels
  return <img src={src} alt="" className="pm-param-icon" draggable={false} />;
}
