...
export default function LiveActivityBar({ locale, plantId = "plant-01" }: { locale: AppLocale; plantId?: string }) {
  ...
  const state: StreamState = requestOk === null ? "connecting" : !requestOk || ageSeconds == null ? "offline" : ageSeconds <= 30 ? "live" : ageSeconds <= 120 ? "stale" : "offline";
  const latest = payload?.latest;
  const label = locale === "id"
    ? { connecting: "MENGHUBUNGKAN", live: "SENSOR LANGSUNG", stale: "DATA TERLAMBAT", offline: "SENSOR OFFLINE" }[state]
    : { connecting: "CONNECTING", live: "LIVE SENSOR", stale: "STALE DATA", offline: "SENSOR OFFLINE" }[state];

  return <aside className={`pm-live-activity is-${state}`} aria-label={locale === "id" ? "Aktivitas sensor langsung" : "Live sensor activity"}>
    <div className="pm-live-source"><i key={pulse} aria-hidden="true" /><strong>{label}</strong><span>{ageSeconds == null ? "—" : `${ageSeconds.toFixed(1)}s`}</span></div>
    <div className="pm-live-values" aria-live="polite">
      <span>🌡 {shown(latest?.temperature, "°")}</span><span>💧 {shown(latest?.humidity, "%")}</span><span>🧪 {shown(latest?.soil_ph, "")}</span><span>☀ {shown(latest?.light, "%")}</span>
    </div>
    <small>{locale === "id" ? `${payload?.history?.length ?? 0} sampel · 60 mnt` : `${payload?.history?.length ?? 0} samples · 60 min`}</small>
  </aside>;
}
