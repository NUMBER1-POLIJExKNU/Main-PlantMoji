...
export async function GET(request: Request) {
  ...
  const questions = dailyQuiz(plantId, locale, `${quizDate}:round:${round}`).map((question) => ({ key:question.key,category:question.category,question:question.question,choices:question.choices }));
  const supabase = getServerSupabase();
  if (!supabase) return Response.json({ ok:true,quizDate,questions,progress:[],offline:true });
  const { data, error } = await supabase.from("daily_quiz_attempts").select("question_key,attempts,completed_at,xp_awarded").eq("plant_id",plantId).eq("quiz_date",quizDate).eq("round_no",round);
  // Migration-free fallback: questions still work visually, but XP is disabled.
  return Response.json({ ok:true,quizDate,questions,progress:error ? [] : data ?? [],offline:Boolean(error) });
}
...
export async function POST(request: Request) {
  ...
  return Response.json({ok:true,...data,explanation:localized.explanation});
}
